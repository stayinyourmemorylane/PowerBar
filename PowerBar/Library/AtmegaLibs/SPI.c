...
void SPI_MasterInit(void){
	...
	DDR_SPI |= (1 << SPI_SS) | (1<< SPI_CLK) | (1 << SPI_MOSI) & ~(1 << SPI_MISO);
	//DDR_SPI = 0b11101100;
	//SPCR = 0x50;
	/*enable SPI, Master, set clock rate fck/16 */
	//SPSR &= (0 << SPI2X);   // double speed this tends to be troublesome it works most of the time but....
	//SPCR = 0x50;
	SPCR &= (0<< SPR0) & (0 << SPR1) & (0 << SPI2X); 
	SPCR |= (0 << DORD) | (0 << CPOL) | (0  << CPHA); // pg 173 of data sheet
	SPCR |= (1 << SPE)| (1<< MSTR);
